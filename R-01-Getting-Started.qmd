...
I highly recommend to install Positron. For installation just follow the instructions on  
...
1. The editor (center) showing your script file, which is an ordinary text file with suffix `.R`. For instance, `yourFavoritFileName.R`. This file contains your code.
...
You can install the packages with `install.packages(packagename)`. In order to use the package type `library(package)`. The package has to be installed only once, but you have to load the package with `library(package)` every time you want to use it.
